...
module.exports["op"] = {
    ...
    "basic operators": function (test) {
        ...
        cases.forEach(op => {
            ...
            test.deepEqual(node.compile2(new LoContext(), new JsStmt()).renderTree(), [ op[1], [ 'num', '1' ], [ 'num', '2' ] ]);
        });
    ...
    "in operator": function (test) {
        ...
        test.deepEqual(node.compile2(new LoContext(), new JsStmt()).renderTree(), [ 'call',
            [ 'select', [ 'id', 'Util' ], 'in' ],
    ...
    "equality is strict": function (test) {
        ...
        test.deepEqual(node.compile2(new LoContext(), new JsStmt()).renderTree(), [ 'strict-eq', [ 'id', '$foo' ], [ 'id', '$bar' ] ]);
        test.done();
    },
