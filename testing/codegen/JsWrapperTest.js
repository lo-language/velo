...
module.exports["basics"] = {
    ...
    "simple passthrough": function (test) {
        ...
        var expr = new JsWrapper(function (resolver) {
            return '(' + resolver.resolve('42') + ')';
        });

        test.equal(new Resolver(expr).render(), '(42)');
        test.done();
    },
    ...
    "simple deferred": function (test) {
        ...
        test.equal(new Resolver(expr).render(), 'Q.spread([foo()], function (ph0) {\nph0;\n}, result.reject);');
        test.done();
    },
    ...
    "as statement": function (test) {
        ...
        var expr = new JsWrapper(function (resolver) {
            return 'bar(' + resolver.resolve(new JsWrapper(function (resolver) {
                    return 'foo()';
                }, true)) + ');';
        }, true);
    ...
    "nested deferred": function (test) {
        ...
        test.equal(new Resolver(expr).render(), 'Q.spread([bar()], function (ph0) {\nfoo([ph0]);\n}, result.reject);');
        test.done();
    },
    ...
    "multiple deferred": function (test) {
        ...
        test.equal(new Resolver(expr).render(), 'Q.spread([foo(),bar()], function (ph0,ph1) {\nQ.spread([baz(ph0, ph1)], function (ph0) {\nph0;\n}, result.reject);\n}, result.reject);');
        test.done();
    },
